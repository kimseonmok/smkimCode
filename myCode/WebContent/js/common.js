...
//			}
	...
	// 저장 결과는 callback 함수를 정의하여 처리한다.
	//================================================
	function IBSpostJsonNoMsg(urls, savejson, param, callback) {
		var ajaxurl = urls;
		if(!isBlankStr(param)) {
			ajaxurl = urls + "?" +param;
		}
	...
	//================================================
	//IBS 그리드 리스트 승인(반려) 처리 후 콜백함수...
	//================================================
	function ibscallbackNoMsg(res){
...
//		},
		error: function (jqXHR, textStatus, errorThrown) {
			...
			var res ;
			if (jqXHR.status == 401) {
				res = {RESULT : {CODE:jqXHR.status, MESSAGE:'로그인 정보가 없습니다.<br>로그인 화면으로 이동하시겠습니까?'}};
			} else {
				res = {RESULT : {CODE:-1, MESSAGE:'시스템을 이용할수 없습니다.<br>관리자에게 문의하세요.'}};
			}
			ibscallback(res);
		}
	});
...
//		if (grid_vrf != null && grid_vrf != undefined)
	 ...
	 function getRqstChg(param, subInfo) {
		 param=param+"&subInfo=" + subInfo;
//		 alert("변경항목 상세 조회중 파람  : " + param);
		 if(subInfo == "COL") {
